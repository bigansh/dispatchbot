...
/**
 * This function handles the reaction to the group VC request and its procedures.
 *
...
const gvcReactionHandler = async (client, origin, user, message) => {
	try {
		...
		if (origin.emoji.name === '❌' && mentions.includes(user.id))
			approval = false
		else if (origin.emoji.name === '✅' && mentions.includes(user.id))
			approval = true
		...
		members.forEach((member) => {
			if (!message.guild.member(member).voice.channelID) connected = false
			else connected = true
		})
...
module.exports = gvcReactionHandler
